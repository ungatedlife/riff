/// Vault-wide [[wiki-link]] targets: every markdown note in the configured
/// Obsidian vault, searchable by name so a riff can link to people, ideas,
/// and older notes — and the links are live the moment it publishes.
use serde::Serialize;
...
/// the first seen, since [[links]] resolve by name anyway.
...
pub fn search_vault_links(query: String) -> Result<Vec<VaultLink>, String> {
    ...
    Ok(rank(scan_vault(&vault_dir), &query))
}

#[cfg(test)]
mod tests {
    use super::{collect_md_notes, rank, VaultLink};
    use std::fs;
